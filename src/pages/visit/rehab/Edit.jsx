...
export default function Edit() {
  ...
  const { id: visitId, actionId, rehabId } = useParams()
  ...
  /**
   * Mengambil data edit rehab
   */
  const rehabQuery = useQuery(`visit-${visitId}-rehab-${rehabId}`, async () =>
    axios.get(`/visit/rehab/${rehabId}/edit`).then((res) => res.data)
  )
  ...
  /**
   * Mengambil data permintaan rehab
   */
  const { data: actionData, status: actionStatus } = useQuery(
    [`visit-${visitId}-rehab-${rehabId}-action`, filter],
    async (key) =>
      axios
  ...
  /**
   * Mengupdate urutan permintaan rehab
   */
  const { mutate } = useMutation(
    async (data) => {
      ...
      return axios
        .post(
          `visit/rehab/${rehabId}/measure/${data.actionId}/update-order`,
          data
        )
        .then((res) => res.data)
    },
  ...
  return (
    <>
                ...
                <div className="card-header border-0 pt-6">
                  <div className="card-toolbar">
                    ...
                    >
                      <Link
                        type="button"
                        to={rehabData?.status === "done" ? "" : "action/create"}
                        className={clsx({
                          "btn btn-primary": true,
                          "disabled cursor-not-allowed":
                            rehabData?.status === "done",
                        })}
                      >
                        <PlusIcon />
                        Tambah
                      ...
                      <div className="table-responsive">
                        <table className="table">
                          ...
                          <tbody>
                            ...
                            {actionData?.data?.map((action, key) => {
                              const dynamicBg =
                                action.status === "finish"
                                  ? "#ecf7f6"
                                  : rehabData?.status === "cancel"
                                  ? "#fbebf0"
                                  : ""
      ...
      {location.pathname.includes(`action/${actionId}/delete`) && (
        <DeleteModal
          url={`/visit/rehab/${rehabId}/measure/${actionId}/destroy`}
          query={`visit-${visitId}-rehab-${rehabId}-action`}
...
/**
 * Komponen komen halaman edit rehab
 */
function Comment() {
  const queryClient = useQueryClient()
              ...
              {commentData.length === 0 ? (
                <div className="d-flex flex-column flex-center min-h-200px">
                  ...
                  <p className="w-200px text-center text-gray-600">
                    Belum ada komentar masuk dari rehab
                  </p>
                </div>
              ) : (
                <>
                  {commentData.map(
                    ...
                    (comment, key) => {
                      if (
                        !comment?.user_role?.toLowerCase().includes("rehab")
                      ) {
                        return (
...
/**
 * Komponen toolbar halaman edit rehab
 * @param {{
 * laboratoryQuery: import("react-query").UseQueryResult
 * }}
 */
function Toolbar({ rehabQuery }) {
  const navigate = useNavigate()
  const rehabData = rehabQuery.data
      ...
      <div className="container-fluid">
        <div className="row justify-content-between py-2 py-md-0">
          <div className="col-sm-9 d-flex align-items-center mb-3 mb-sm-0 text-truncate">
            ...
            {rehabStatus === "success" && (
              <>
                <h1 className="fs-3 mb-0 me-4">{rehabData.unique_id}</h1>
                <RenameStatus status={rehabData.status} />
              </>
            )}
          </div>
          <div
          ...
          >
            {rehabStatus === "success" && (
              <UpdateStatusButton
                ...
                currentStatus={rehabData.status}
              />
            )}
            <ButtonDetailPatient />
          </div>
...
/**
 * Tombol untuk mengupdate status rehab
 * @param {{
 * id: Number
...
export function UpdateStatusButton({ id, currentStatus }) {
  ...
  return (
    <>
      {location.pathname.includes(`/rehab/${id}/edit/status/${status}`) && (
        <UpdateModal
          url={`/visit/rehab/${id}/update-status`}
          data={{ status: status }}
          query={`visit-${visitId}-rehab-${rehabId}`}
          message={`
