...
// komponen utama Diagnosa 9
export default function EditDiagnosaNine() {
  const navigate = useNavigate()
  const location = useLocation()
  const { id: visitId, diagnosaNineId } = useParams()
  ...
  // get listing data diagnosa 9
  const { data: diagnosaTenData, status: diagnosaTenStatus } = useQuery(
    [`visit-${visitId}-icd-nine-listing`, filter],
    async (key) =>
      axios
        .get(`/visit/${visitId}/icd-nine/listing`, {
  ...
  return (
    <>
                  ...
                  <div className="card-toolbar">
                    ...
                    >
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            !allPermissions.includes("tambah icd 9")
                              ? ""
                              : "create"
                          )
                        }
                        disabled={!allPermissions.includes("tambah icd 9")}
                  ...
                  {diagnosaTenStatus === "success" && (
                    <div className="position-relative">
                      ...
                      >
                        {diagnosaTenData.data.map((v, key) => (
                          <ButtonLightStyled
                            ...
                            disabled={
                              v.is_add === 0 ||
                              !allPermissions.includes("ubah icd 9")
                            }
                          >
                            <PrimaryPencilIcon />
                          </ButtonLightStyled>
                      ...
                      <div className="table-responsive">
                        <table
                          className="table"
                          style={{ marginLeft: "80px", minWidth: "800px" }}
                        >
                          <TableHeading
                                  ...
                                  <td className="align-middle">
                                    <div className="d-flex">
                                      <CancelIcdButton
                                        id={data.id}
                                        name={data.name}
                                        query={`visit-${visitId}-icd-nine-listing`}
                                        isAdd={data.is_add}
                                      />
                                    </div>
                                  </td>
      ...
      {location.pathname.includes(`create`) && (
        <Create visitId={visitId} query={`visit-${visitId}-icd-nine-listing`} />
      )}
      {location.pathname.includes(`edit`) && (
        <Edit
          diagnosaNineId={diagnosaNineId}
          query={`visit-${visitId}-icd-nine-listing`}
        />
      )}
    </>
  ...
  return (
    <>
      {location.pathname.includes(`${id}/delete`) && (
        <DeleteModalMethode
          url={`visit/icd-nine/${id}/update-status`}
          query={query}
          message={`
        ...
        >
          <Dropdown.Menu show={true} className="py-5 w-200px">
            <DropdownStatusStyled
              disabled={!allPermissions.includes("ubah icd 9") || isAdd === 0}
            >
              <Dropdown.Item
                disabled={true}
...
export const DropdownItemStyledIcd = styled.div`
  span {
    color: #7e8299;
  }
